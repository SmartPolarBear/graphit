...
inline indices make_row_major_order(std::size_t num_dims) {
    indices result = ranges::to<indices>(ranges::view::iota(0ul) | ranges::view::take(num_dims) | ranges::view::reverse);
    return result;
}

inline indices make_col_major_order(std::size_t num_dims) {
    indices result = ranges::to<indices>(ranges::view::iota(0) | ranges::view::take(num_dims));
    return result;
}
